 ...
 * 
 * 在MobileApi里面的接口有些是不要验证的接口，有些是需要登录了才能使用的，这个apiserver 专门负责这个复杂的关系
 *
...
class MobileApiServer
{
   ...
   protected function initApiWhiteList()
   {
      $this->whiteList = array(
         'User.login',
         'User.checkPhoneExist',  //验证手机号是否已使用
         'User.sendSmsCode',  //发送短信验证码
         'User.register',    //注册  
         'User.findPassword'
      );
   }

   protected function initApiBlackList()
   {
   ...
   {
      //不需要验证
      if (self::AUTH_LEVEL_1 == $authType && !in_array($key, $this->blackList)) {
         return true;
      }
      //首先验证是否在白名单里面
      ...
      //验证登录
      if (self::AUTH_LEVEL_2 == $authType || in_array($key, $this->blackList)) {
         //需要验证登录
