 ...
 */
Ext.define('App.Site.CmMgr.Ui.FieldTypeWin', {
    ...
    setTargetType : function(type, uiOption)
    {
        ...
        var cls = 'App.Site.CmMgr.Lib.FieldOptSetter.' + type;
            ...
            listeners : {
                ...
                itemclick : function(panel, record)
                {
                   ...
                   if(this.mode == CloudController.Const.MODIFY_MODE){
                      uiOption = this.fieldModel.uiOption;
                   }
                    //在这个里面选择的不可能带有参数
                    this.setTargetType(record.get('type'), uiOption);
                },
                scope : this
            }
        };
    ...
    getTypeStore : function()
    {
        ...
        var protectedFields = this.getProtectedFieldType();
        var item;
        ...
        for(var type in TYPE) {
            ...
            if(this.mode == this.self.M_NEW){
                if(Ext.Array.contains(protectedFields, type)){
                    item.disabled = true;
                }
     ...
     * 出现在这个里面的类型都不能选择
     */
    getProtectedFieldType : function()
    {
        return [
            'Title'
        ];
