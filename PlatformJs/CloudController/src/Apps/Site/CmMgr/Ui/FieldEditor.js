 ...
 *
 */
Ext.define('App.Site.CmMgr.Ui.FieldEditor', {
    extend: 'Ext.form.Panel',
    requires: [
        'Cntysoft.Utils.HtmlTpl',
        'App.Site.CmMgr.Ui.FieldTypeWin'
    ],
    mixins: {
    ...
    fieldUiOpt : {},
    /*
     * @property {App.Site.CmMgr.Ui.FieldTypeWin} fieldTypeWinRef
     */
    ...
    saveFieldHandler : function()
    {
        ...
        if(form.isValid()){
            ...
            if(this.mode == CloudController.Const.NEW_MODE){
                this.appRef.addField(this.targetModel.modelKey, values, function(response){
                    this.loadMask.hide();
                    if(!response.status){
    ...
    openFieldTypeWindowHandler : function()
    {
        ...
        if(!this.fieldTypeWinRef){
            //获取相关的值
            this.fieldTypeWinRef = new App.Site.CmMgr.Ui.FieldTypeWin({
                mode : this.mode,
    ...
    fieldsConfig : function()
    {
        ...
        return [{
            ...
            name : 'name',
            validateBlank : true
            //validator : Ext.bind(this.checkFieldKeyHandler, this)
        }, {
            xtype : 'textfield',
