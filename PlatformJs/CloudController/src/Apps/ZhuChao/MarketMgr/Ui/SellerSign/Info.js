 ...
 */
/**
 * 该类生成弹出窗口，显示单个入驻商家的详细信息
 * 
 * @param {type} param1
 * @param {type} param2
 */
Ext.define('App.ZhuChao.MarketMgr.Ui.SellerSign.Info', {
   extend : 'Ext.window.Window',
   alias : 'widget.appzhuchaomarketmgruisellersigninfo',
   mixins : {
   ...
   {
      config = config || {};
      this.LANG_TEXT = this.GET_LANG_TEXT('SELLERSIGN.INFO');
   ...
   {
      this.appRef.sellerSignRead(this.record.getId(), function (response){
         if(!response.status){
            Cntysoft.showErrorWindow(response.msg);
         } else{
            this.data = response.data;
            this.data.address = this.data.privince + this.data.city + this.data.address + this.data.part + this.data.floor + this.data.dis + this.data.num;
      ...
      Ext.apply(this, {
         items : [{
               xtype : 'form',
               items : [{
                     fieldLabel : this.LANG_TEXT.SELLERNAME,
                     name : 'sellerName',
                     readOnly : true,
                     xtype : 'textfield'
                  }, {
                     ...
                     fieldLabel : this.LANG_TEXT.SIGNTIME,
                     name : 'signTime',
                     readOnly : true,
                     xtype : 'textfield'
                  }, {
   ...
   windowButtonClickHandler : function (button)
   {
      ...
      this.appRef.sellerSignContextMenuClick(res, function (response){
         if(!response.status){
            Cntysoft.showErrorWindow(response.msg);
         } else{
