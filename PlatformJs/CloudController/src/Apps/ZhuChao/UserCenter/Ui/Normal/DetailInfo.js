 ...
 */
Ext.define('App.ZhuChao.UserCenter.Ui.Normal.DetailInfo', {
    ...
    {
        config = config || {};
        this.LANG_TEXT = this.GET_LANG_TEXT('UI.NORMAL.DETAIL_INFO');
    ...
    getInfoValues : function ()
    {
        ...
        if(!this.rendered && this.responseData){
            var list = ['province', 'city', 'district', 'address', 'qq', 'sex'];
            var len = list.length;
            for(var i = 0; i < len; i++) {
                var name = list[i];
