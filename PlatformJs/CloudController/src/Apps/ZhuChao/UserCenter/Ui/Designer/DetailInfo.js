 ...
 */
Ext.define('App.ZhuChao.UserCenter.Ui.Designer.DetailInfo', {
    ...
    statics : {
        ...
        TYPE_KEY : 'ZhuChao.UserCenter.Designer.Type',
        MAX_VALUE_LEN : 5
    },
    /*
    ...
    {
        config = config || {};
        this.LANG_TEXT = this.GET_LANG_TEXT('UI.DESIGNER.DETAIL_INFO');
    ...
    getItemsConfig : function ()
    {
        ...
        return [{
                    ...
                    afterrender : function (checkbox){
                        this.mainPanelRef.appRef.getSysKvDict(this.self.STYLE_KEY, function (response){
                            ...
                            Ext.each(data, function (item){
                                ...
                                items.push({
                                    boxLabel : item['name'], name : 'style', inputValue : item['value'], checked : checked
                                });
                            }, this);
                            checkbox.add(items);
                            //为每个checkbox添加事件,保证
                                ...
                                item.addListener({
                                    change : function (box, value){
                                        if(value){
                                            ...
                                            //判断长度是否超过5个
                                            if(this.self.MAX_VALUE_LEN < values['style'].length){
                ...
                listeners : {
                    afterrender : function (checkbox){
                        this.mainPanelRef.appRef.getSysKvDict(this.self.TYPE_KEY, function (response){
                            var data = response.data;
                            var type = [];
                            if(this.responseData){
                                type = this.responseData['designType'];
                            }

                            var items = [];
                            Ext.each(data, function (item){
                                var checked = Ext.Array.contains(type, item['value']) ? true : false;
                                items.push({
                                    boxLabel : item['name'], name : 'designType', inputValue : item['value'], checked : checked
                                });
                            }, this);
                            checkbox.add(items);
                            checkbox.items.each(function (item){
                                item.addListener({
                                    change : function (box, value){
                                        if(value){
                                            var values = checkbox.getValue();
                                            //判断长度是否超过5个
                                            if(this.self.MAX_VALUE_LEN < values['designType'].length){
                                                Cntysoft.showAlertWindow(this.GET_LANG_TEXT('UI.MSG.OVER_LENGTH'), function (){
                                                    box.setValue(false);
                                                }, this);
                                            }
    ...
    getInfoValues : function ()
    {
        ...
        if(!this.rendered && this.responseData){
            var list = ['province', 'city', 'district', 'address', 'qq', 'sex', 'style', 'designType', 'intro', 'serviceProvince', 'serviceProvince', 'experience'];
            var len = list.length;
            for(var i = 0; i < len; i++) {
                var name = list[i];
    ...
    applyInfoValue : function (values)
    {
        if(!this.rendered){
            ...
            var filter = ['province', 'city', 'district', 'serviceProvice', 'serviceCity'];
