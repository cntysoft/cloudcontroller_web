 ...
 * 图片选择器
 */
Ext.define('App.ZhuChao.DeliveryOrderUse.Comp.ImageGroup', {
   extend: 'Ext.form.FieldContainer',
   xtype : 'usercenterdeliveryorderusecompimagegroup',
   requires: [
      'App.ZhuChao.DeliveryOrderUse.Comp.IconView',
      'Ext.menu.Menu',
   ...
   runableLangKey : 'App.ZhuChao.DeliveryOrderUse',
   ...
   {
      config = config || {};
      this.LANG_TEXT = this.GET_LANG_TEXT('COMP.IMAGE_GROUP');
...
//      });
      ...
      Ext.require('WebOs.Component.Uploader.Window', function(){
         ...
         if(null == this.uploadWinRef){
            Ext.apply(uploaderConfig, {
               initUploadPath : FH.getAppUploadFilesPath('ZhuChao', 'DeliveryOrderUse'),
               uploaderConfig : {
                  //启用附件追踪
                  enableFileRef : true,
   ...
   getImageViewConfig : function()
   {
      return {
         ...
         autoScroll : true,
         items : {
            xtype : 'zhuchaodeliveryorderusecompiconview',
            iconHeight : 160,
            iconWidth : 120,
